...
class Wallet extends Component {
  ...
  BalanceUserId = () => {
    const { action, user } = this.props;
    ApiServices.userId({ _id: user._id })
      .then(res => {
        if (res.code === 200) {
  ...
  renderItem = ({ item, index }) => {
    ...
    const dataBalance = [
      {
        name: 'BFI',
        amount: balanceUserid.bfi
          ? balanceUserid.bfi.toLocaleString('en-US', {
              // style: 'currency',
              // currency: 'USD',
            })
          : '0.00',
      ...
      {
        name: 'PAY',
        amount: balanceUserid.usdt
          ? balanceUserid.usdt.toLocaleString('en-US', {
              // style: 'currency',
              // currency: 'USD',
            })
          : '0.00',
      ...
      {
        name: 'PAY',
        amount: balanceUserid.pay
          ? balanceUserid.pay.toLocaleString('en-US', {
              // style: 'currency',
              // currency: 'USD',
            })
          : '0.00',
  ...
  render() {
    ...
    const balanceUserid = userId.data;
    const totalBalanceUsd = balanceUserid.bfi * rate + balanceUserid.usdt;
    const totalBalancemer = totalBalanceUsd / rate;
    // console.log('balanceUserid', userId)
    const dataBalance = [
      {
        name: 'BFI',
        amount: balanceUserid.bfi
      ...
      // {
      //     name: "PAY", amount: balanceUserid.usdt ? (balanceUserid.usdt).toLocaleString('en-US', {
      //         // style: 'currency',
      //         // currency: 'USD',
      //     }) : "0.00", icon: "ic_coin_t"
      // },
    ...
    return (
      <SafeAreaView
              ...
              <View
                ...
                <TouchableOpacity
                  ...
                  <View
                    style={{
                      height: '100%',
                      backgroundColor: isSpending ? '#2EDBDC' : '#FFFFFF',
                      paddingVertical: 4,
                      width: '100%',
                      alignItems: 'center',
                ...
                <TouchableOpacity
                  ...
                  <View
                    style={{
                      height: '100%',
                      backgroundColor: isWallet ? '#2EDBDC' : '#FFFFFF',
                      paddingVertical: 4,
                      width: '100%',
                      alignItems: 'center',
                      justifyContent: 'space-around',
        ...
        <View
          ...
          <TouchableOpacity
            onPress={() => this.props.navigation.navigate(stackNavigator.SWAP)}
          ...
          <TouchableOpacity
            onPress={() =>
              this.props.navigation.navigate(stackNavigator.WALLET_PASSCODE)
            }
            disabled={this.state.disabledTouch}
            style={{
              alignItems: 'center',
