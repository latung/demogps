...
class Transfer extends Component {
  constructor(props) {
    super(props);
    this.bottomSheetRef = new createRef();
    ...
    this.state = {
      ...
      toggle: false,
      isTransfer: false,
      tokenType: { name: 'BFI', icon: 'ic_location', next: false },
    };
  }
  ...
  transfer = () => {
    const { from, to, tokenType } = this.state;
      ...
      () => {
        if (from.type === 'isWallet') {
          ApiServices.transfer({
            userId: user._id,
            amount: from.value,
            type: tokenType.name.toLowerCase(),
          })
            .then(res => {
              alert(res.message);
              if (res.code === 200) {
  ...
  render() {
    ...
    const dataBS = from.type === 'isWallet' ? dataWallet : dataSpend;
    return (
                        ...
                        <View
                          ...
                          <TextInput
                            ...
                            onChangeText={itemValue =>
                              this.onChangeText('from', itemValue)
                            }
                            style={{
                              fontWeight: 'bold',
                              fontStyle: 'italic',
